...
class Mod_brand extends CI_Model
{
	...
	function insert_brand($table, $data)
    {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('aset_merek', $data);
    }

        function get_brand($id)
    {   
        $this->db->where('id',$id);
        return $this->db->get('aset_merek')->row();
    }

        function delete_brand($id, $table)
    {
        $this->db->where('id', $id);
