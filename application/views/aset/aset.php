      ...
      <div class="col-12">
        <div class="card">
          <div class="card-header bg-light">
            <h3 class="card-title"><i class="fa fa-list text-blue"></i> Data Aset</h3>
            <div class="text-right">
              <a href="<?= base_url('aset/download'); ?>" type="button" class="btn btn-sm btn-outline-info" title="Download" target="_blank"><i class="fas fa-download"></i> Download</a>
              <button type="button" class="btn btn-sm btn-outline-primary" onclick="add_ast()" title="Add Data"><i class="fas fa-plus"></i> Add</button>
            </div>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="tbl_aset" class="table table-bordered table-striped table-hover">
              <thead>
                <tr class="bg-info">
                  <th>Picture</th>
                  <th>QR</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    //datatables
    table = $("#tbl_aset").DataTable({
      "responsive": true,
      "autoWidth": false,
      "language": {
        "sEmptyTable": "Data aset Belum Ada"
      },
      ...
      "columnDefs": [{
          ...
          "render": function(data, type, row) {
            ...
            return "<a class=\"btn btn-xs btn-outline-primary\" href=\"javascript:void(0)\" title=\"Edit\" onclick=\"edit_aset(" + row[9] + ")\"><i class=\"fas fa-edit\"></i></a>";
        ...
        },
        {
          ...
          "render": function(data, type, row) {
            if (row[8] != null) {
              return "<img class=\"myImgx\"  src='<?php echo base_url("asets/foto/foto/"); ?>" + row[8] + "' width=\"100px\" height=\"100px\">";
            } else {
              return "<img class=\"myImgx\"  src='<?php echo base_url("asets/foto/foto/Logo.png"); ?>' width=\"100px\" height=\"100px\">";
            }
          }
        },
      ],
  ...
  function edit_aset(id) {
    ...
    $.ajax({
      url: "<?php echo site_url('aset/edit_aset') ?>/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        ...
        if (data.logo == null) {
          var image = "<?php echo base_url('asets/foto/logo/Logo.png') ?>";
          $("#v_image").attr("src", image);
        } else {
          var image = "<?php echo base_url('asets/foto/logo/') ?>";
  ...
  function add_ast() {
    ...
    $('.modal-title').text('Add aset'); // Set Title to Bootstrap modal title
  }

  function edit_ast(id) {
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    $('.help-block').empty(); // clear error string
  ...
  function save() {
    ...
    var url = "<?php echo site_url('aset/update') ?>";
          ...
          <div class="card-body">
            <div class="form-group row ">
              ...
              <div class="col-sm-9 kosong">
                <input type="text" class="form-control" name="nama_aset" id="nama_aset" placeholder="Nama aset">
            ...
            <div class="form-group row ">
              ...
              <div class="col-sm-9 kosong">
                <input type="text" class="form-control" name="tahun" id="tahun" placeholder="Nama aset">
                <span class="help-block"></span>
              </div>
            </div>
            <div class="form-group row ">
